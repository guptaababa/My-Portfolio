...
const Projects = () => {
  ...
  return (
    <animated.section ref={ref} id="projects" className="py-10">
      <animated.h2
        style={titleProps}
        className="text-4xl sm:text-6xl font-bold text-center mb-12 text-violet-400"
      >
        My Projects
      </animated.h2>
...
const ProjectCard = ({ project, inView, index }) => {
  const props = useSpring({
    opacity: inView ? 1 : 0,
    transform: inView ? "translateY(0)" : "translateY(50px)",
    delay: inView ? 200 + index * 150 : 0, // staggered cards
  });
  ...
  return (
    <animated.a
      ...
      style={props}
      className="bg-gray-950 p-8 rounded-2xl shadow-lg border border-gray-700 transform hover:scale-105 hover:-translate-y-2 transition-transform duration-300 block"
    >
      <h3 className="text-2xl sm:text-3xl font-bold text-fuchsia-300 mb-3">
        {project.title}
      </h3>
      ...
      <div className="flex flex-wrap gap-2">
        {project.technologies.map((tech, i) => (
          <span
            ...
            text-gray-300 px-4 py-2 rounded-full text-sm font-semibold"
          >
            {tech}
          </span>
        ))}
